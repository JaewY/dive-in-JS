...
### 📌 심벌이란?
- 심벌은 ES6에서 도입된 7번째 데이터타입으로 변경 불가능한 원시 타입의 값이다.
...
// Symbol 함수를 호출하여 생성한 심벌 값은 전역 심벌 레지스트리에 등록되어 관리되지 않는다ㅏ.
const s2 = Symbol('foo');
// 전역 심벌 레지스트리에 저장된 심벌 값의 키를 추출
...
### 📌 심벌과 상수
- 상수 값 1, 2, 3, 4가 변경될 수 있으며, 다른 변수 값과 중복될 수도 있다는 문제가 있다.
